    ...
    return (
        <div className="btn-group lg:btn-group-horizontal flex align-middle justify-center my-10">
            {props.tabs.map((tab: Tab, key:number) => (
                <button key={key} className={`btn ${props.currentTab === tab.id && "btn-secondary"} ${tab.disabled && "btn-disabled"}`} onClick={() => props.setCurrentTab(tab.id)}> {tab.button_name}</button>
            ))}
        </div>
    )
}